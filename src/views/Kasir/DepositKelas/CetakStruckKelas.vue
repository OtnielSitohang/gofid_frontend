...
<template>
    <div class="ma-12">
        <v-row>
            ...
            <v-col align="right">
                <v-btn class="ma-2" color="primary" to="../DepositKelas"><v-icon>mdi-plus</v-icon></v-btn>
            </v-col>
        </v-row>
        <template>
            <v-data-table :headers="headers" :items="DepositeKelasController">
...
export default {
    ...
    data() {
        return {
            ...
            StruckDialog: false,
            headers: [
                { text: "No Struck", value: "NO_STRUK_DEPOSIT_KELAS" },
                { text: "ID MEMBER", value: "ID_MEMBER" },
                { text: "ID PEGAWAI", value: "ID_PEGAWAI" },
                { text: "KELAS", value: "NAMA_KELAS"},
                { text: "JUMLAH DEPOSITE", value: "JUMLAH_DEPOSIT_KELAS" },
                { text: "BONUS", value: "BONUS_DEPOSIT_KELAS" },
                { text: "TANGGAL DEPOSIT", value: "TANGGAL_DEPOSIT_KELAS" },
                { text: "TANGGAL KADALUARSA", value: "TANGGAL_KADALUARSA_DEPOSIT_KEL" },
                { text: "TOTAL DEPOSIT", value: "JUMLAH_DEPOSIT_KELAS" },
    ...
    setup() {
        let DepositeKelasController = ref([]);
        let KelasController = ref([]);
        onMounted(() => {
            axios.get('http://localhost:8000/api/indexDepositeKelas')
                .then(response => {
                    DepositeKelasController.value = response.data.data
                    // console.log(DepositeKelasController.value);
                }).catch(error => {
        ...
        function displayCetakStruck(item) {
            ...
            let SelectedMember = this.KelasController.filter(i => i.ID_KELAS == item.ID_KELAS);
            this.NAMA_KELAS = SelectedMember[0].NAMA_KELAS;
            ...
            setTimeout(() => {
                ...
                ctx.fillText('No Struck     : ' + item.NO_STRUK_DEPOSIT_KELAS, 700, 60)
                ctx.fillText('Tanggal        : ' + item.TANGGAL_DEPOSIT_KELAS, 700, 95)

                ctx.fillText('MEMBER ID               : ' + item.ID_MEMBER, 30, 200)
                ctx.fillText('JENIS KELAS             : ' + this.NAMA_KELAS, 30, 240)
                ctx.fillText('BONUS DEPOSIT       : ' + item.BONUS_DEPOSIT_KELAS, 30, 280)
                ctx.fillText('BERLAKU SAMPAI   : ' + item.TANGGAL_KADALUARSA_DEPOSIT_KEL, 30, 320)
