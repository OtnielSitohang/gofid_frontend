...
<template>
    <div class="ma-12">
        ...
        <template>
            <v-data-table :headers="headers" :items="IndexKelas" :search="search">

                <template v-slot:item.actions="{ item }">
                    <v-btn x-small class="mr-2" @click="openVerifDialog(item)"> RESET
...
export default {
    ...
    data() {
        return {
            ...
            search: '',
            headers: [
                { text: "NO STRUCK DEPO", value: "NO_STRUK_DEPOSIT_KELAS" },
                { text: "NAMA KELAS", value: "NAMA_KELAS" },
                { text: "ID MEMBER", value: "ID_MEMBER" },
                { text: "NAMA USER", value: "NAMA_USER"},
                { text: "TANGGAL KADALUARSA DEPOSIT KEL", value: "TANGGAL_KADALUARSA_DEPOSIT_KEL"},
    ...
    methods: {
        ...
        DekativasiMemberProcess() {
            axios.put('http://localhost:8000/api/ResetDepositeKelas/' + this.DeaktivasiTarget.MEM_ID_USER)
            .then(response => {
                    // console.log(this.DeaktivasiTarget.MEM_ID_USER)
                    this.users = response.data.data;
                    toastr.success('Kelas Reset Succes');
                    // window.location.reload();
                })
                .catch(error => {
                    console.log(error);
                    toastr.error('User Reset failed');
                });
            this.VerifDialog = false;
        },
    },
    ...
    setup() {
        let IndexKelas = ref([]);
        onMounted(() => {
            axios.get('http://localhost:8000/api/indexResetDepositeKelas')
                .then(response => {
                    IndexKelas.value = response.data.data
                    // console.log(IndexKelas.value[0].ID_MEMBER)
                }).catch(error => {
